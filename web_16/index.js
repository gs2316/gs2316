// 휴대폰 번호 입력 부분  constant 변함없는 = 상수 , document -> html 문서 자체를 의미
function changePhone1(){
    const phone1 = document.getElementById("phone1").value // 010
    if(phone1.length === 3){   // == 값이 같은지, === 값과 타입이 같은지
...
// 문자인증+타이머 부분
function initButton(){
  document.getElementById("sendMessage").disabled = true; //disabled 비활성화
  document.getElementById("completion").disabled = true;
  document.getElementById("certificationNumber").innerHTML = "000000"; //innerHTML 기존 내용을 새 내용으로 대체
  document.getElementById("timeLimit").innerHTML = "03:00";
  document.getElementById("sendMessage").setAttribute("style","background-color:none;") //setAttribute 속성값을 새롭게 설정
  document.getElementById("completion").setAttribute("style","background-color:none;")
}

let processID = -1;  // 변수 선언 

const getToken = () => {  // => 화살표 함수
  ...
  processID = setInterval(function () {
    ...
    let mm = String(Math.floor(time / 60)).padStart(2, "0");  // padStart(자리, "0"으로 채우기)
    let ss = String(time % 60).padStart(2, "0");  // % 나누고 나머지
    let result = mm + ":" + ss;
